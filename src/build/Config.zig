...
pub fn init(b: *std.Build, appVersion: []const u8, libVersion: []const u8) !Config {
    ...
    config.version = if (version_string) |v|
        ...
        if (vsn.tag) |tag| {
            // Tip releases and Maxx app release tags (including the old
            // madmaxx-v and mosttly-v prefixes still present in history) are not upstream
            // Ghostty releases, so they behave like any other pre-release.
            if (!std.mem.eql(u8, tag, "tip") and
                !std.mem.startsWith(u8, tag, "maxx-v") and
                !std.mem.startsWith(u8, tag, "madmaxx-v") and
